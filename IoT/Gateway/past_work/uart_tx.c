...
int main(void)
{
	...
	temp = write(uart0_filestream, setChannel, 4);
	printf("%d\n", temp);
	temp = write(uart0_filestream, setTxPower, 3);
	printf("%d\n", temp);
	temp = write(uart0_filestream, setDataRate, 3);
	printf("%d\n", temp);
	temp = write(uart0_filestream, offModule, 2);
	printf("%d\n", temp);
	sleep(1);
	while (1) {
    /*
		...
		*pTxBuffer++ =  0xBB;
		*pTxBuffer++ =  0xCC;
		txLength = write(uart0_filestream, txBuffer, 13);
		if (txLength < 0) {

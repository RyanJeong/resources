...
inline void processReset(void)
{
   ...
   */
  ADCSRA &= ~(1 << 7);
  data[index++] = temp;   /*  store data that from analogRead(A1) that not process filtering  */
  if (index == DATA_LEN) {
    index = 0;
  }
...
inline void processSensing(void)
{
  ...
  ADCSRA &= ~(1 << 7);
    ...
    Serial.print((char) 0xEE);  //  ID
    Serial.print((char) 0x30);  //  transmit
    Serial.print((char) 0x78);
    Serial.print((char) 0x88);  //  CRC
    
  data[index++] = temp;   /*  store data that from analogRead(A1) not process filtering  */
  if (index == DATA_LEN) {
    index = 0;
  }
  filtered2 = (Q1 * temp * temp) + (Q2 * temp) + Q3;

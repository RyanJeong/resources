...
#define STATE_FILE_ALARM        1
#define STATE_SEND_TEST_RESULT  2
#define STATE_RECV_TEST_CMD     3
#define STATE_SEND_DATAS        4
#define GREEN         5
#define YELLOW        6
#define RED           7
...
char  rxLength; /*  rx frame length  */
char  rxID;     /*  rx frame ID  */
char  rxCMD;    /*  rx frame CMD */
char  rxCRC;    /*  rx frame CRC */
...
inline boolean sendTestResult(int _state)
{
  ENABLE_RF();
  WDTCSR &= ~(1 << 6);        /*  Disable interrupt mode */
...
inline boolean receiveTestCommand(void)
{
  ...
  txLength  = 0x03;
  txCMD     = 0x20;
  txCRC     = getCRC0(txLength, txID, txCMD);
  RF_Serial.print((char) 0xAB);
  RF_Serial.print((char) 0x01);
  ...
  if (RF_Serial.available()) {
    rxLength = RF_Serial.read();
    for (i = 0; i < rxLength; ++i) {
      *pRx++ = RF_Serial.read();
    }
  }
  digitalWrite(YELLOW, LOW);    /*  전송 끝, 결과 수신 */
  if ((rxLength > 0) && !getCRC1(rxLength, rxBuf[0], rxBuf[1], rxBuf[2])) {
      digitalWrite(GREEN, HIGH);    /*  전송 끝, CRC 에러 없음  */
      if (rxBuf[1] & 0x0F) {
        test = true;
        Serial.println("Test cmd received!");
      } else {
        test = false;
      }
  } else {
      digitalWrite(RED, HIGH);    /*  전송 끝, CRC 에러 있음 */
  }
  WDTCSR |= (1 << 6);         /*  Enable interrupt mode */

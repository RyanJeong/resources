...
#define BUZZ_REALY      A5   // warning sound 
...
float normal_threshold  = 3.0;    // ~ 2 degree :  for cool test 
float heating_threshold = 0.2;    // ~ 4 degree :  for heat test
float data_in;
...
void setup() {
  ...
  digitalWrite(STATUS1, LOW);  
  pinMode(BUZZ_REALY, OUTPUT);
  digitalWrite(BUZZ_REALY, LOW);  
...
void loop() {         //mode == 0 : 화제감시 , mode == 1 : 테스트
    ...
    if (mode == 0)   //화제감시모드 : surveillance                      
    {
      ...
      float sensor_diff = abs(filtered_data1-filtered_data2);
      if (sensor_diff > 10.0)                  // 온도가 점점 오르고 있음: 주의 경고  
      {
        Serial.print("temp rising warning !");
      }
    ...
    if (mode == 1)   //test mode
      {
        //-----------------normal-------------------------------
            ...
            if ( (heater_temp[2] - sensor_temp[0]) < 2*heating_threshold )
            {
                //call out_of_order;  // 고장 통보 

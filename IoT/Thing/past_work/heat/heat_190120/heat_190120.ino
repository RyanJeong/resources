...
float fire_threshold_temp  = 30;    // fire alarm temperature   [ 60 ~ 80 C] 화재 알람 온도 
float fire_rise_warning    = 27;    // temperature rise warning [ 60 ~ 80 C] 이 온도부터 1초 간격으로 감시 
float fire_threshold_temp_low;
float normal_threshold  = 3.0;      // ~ 3 degree :  for 정상상태 온도 차이값
float heating_threshold = 0.5;      // ~ 0.5 degree :  가열시 온도 상승 차이 최소값
float data_in;
...
{         // 
    ...
    if (mode == 0)   // 화제감시모드 : surveillance                      
    {
      ...
      temp2     = (float) analogRead(A1)/100.0;         // heating tester seanor 
      ...
      {
          delay(1000);       // 1 second sample delay: fire warning  
      }
      else 
      {
          delay(1000);       // 5 second sample delay 
      }
    ...
    if (mode == 1)   // test mode
      {
        //-----------------normal-------------------------------
            ...
            temp2     = (float) analogRead(A1)/100.0;   // heat test sensor 
            ...
            // 가열하면서 동작 확인 --------------------------  
            if ( heater_temp[0] < 45.0 ) {        //   heater 과열 감지 : 45도 이하이면 테스트 시작 
               ...
               if ( (heater_temp[1] - heater_temp[0]) < heating_threshold )
                ...
                Serial.println(" cooling 1 minute");        // 60초 냉각
                ...
                Serial.println(" sensor over heating  ");  // 센서가 과열이라 테스트 모드 무시 
            }  // 45도 이상 

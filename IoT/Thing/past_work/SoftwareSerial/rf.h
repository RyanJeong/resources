 ...
 *          - 동작 우선순위는 Test > LED or Buzzer
 *          - 만약 테스트 중이거나 화재 감지, 재연결 등 일반 상태가 아닌 특수 상태일 경우, 해당 동작들이 끝날 때까지 Test, LED 또는 Buzzer가 지연됨
 ...
 *  sendMsgA(void)
 *  --------------------
 *  Send message to Raspberry Pi when defined TEST_COMM
 */
void sendMsgA(void);
 ...
 *  sendMsgB(void)
 *  --------------------
 *  Send message to Raspberry Pi when defined COMM
 */
void sendMsgB(void);
 ...
 *  recvMsgA(void)
 *  --------------------
 *  Recv. message from Raspberry Pi when defined TEST_COMM 
 */
void recvMsgA(void);
 ...
 *  recvMsgB(void)
 *  --------------------
 *  Recv. message from Raspberry Pi when defined COMM
 */
void recvMsgB(void);

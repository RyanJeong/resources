...
int fire_detection_lapse = 2;     // 화재 지속 판정 유지 time [2 second] 
int reset_lapse_counter  = 60;    //  초기 power ON 시 화재 비감지 시간 [60 second] 
...
float past_threshold = 0.15;      // 전원 ON 초기치 평균값
float smoke_threshold = 0.15;     // 전원 ON 초기치 평균값
float smoke_threshold_high = 0.15;  // [volt]  smoke threshold voltage maximum
float past_data = 0.15;
...
float OFF_threshold = 0.5;               // 0.5 [Volt]: sensor OFF test mode voltage difference 
float ON_threshold  = 1.5;               // 1.5 [Volt]: sensor ON test mode voltage difference 
...
void power_on_reset()          // 처음 전원을 켜면 일정시간 무동작 : 60 seconds
{
  ...
  past_data   = 0.15;          // 연기 초기치
  smoke_threshold = 0.15;      // 연기 초기치
...
void setup() {
  ...
  digitalWrite(Shutter, LOW); 
    ...
    smoke_threshold = 0.15;    // initialize

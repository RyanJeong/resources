...
#define VCC_CHECK   A3      // analog voltage in : battery check 
#define BUZZER      A4      // sound 
#define RELAY       A5      // switch 
...
int reset_lapse_counter  = 10*sample_rate;  //  초기 power ON 시 화재 비감지 시간 [60 second] 
...
float filter_out;
float data_adc;
float past_threshold = 0.5;      // 초기치 평균값
float smoke_voltage;             //  Smoke 센서 ADC값을 5V로 변환
float smoke_threshold = 0.5;     // 초기치 평균값 
float smoke_threshold_high = 0.5;  // [volt]  smoke threshold voltage maximum
float past_data = 0.5;

float smoke_detection_voltage = 0.05;    // [Volt] smoke voltage difference                      
float noise_threshold = 0.05;            // [Volt] for average voltage reference  

float OFF_threshold = 0.8;               // 0.8 [Volt]: sensor OFF test
float ON_threshold  = 2.0;               // 2.0 [Volt]: sensor ON test 
...
void power_on_reset()          // 처음 전원을 켜면 일정시간 무동작 : 60 seconds
{
  float alpha = 0.6;
  float beta  = 1.0-alpha;
  past_data   = 0.7;          // 연기 초기치
  smoke_threshold = 0.7;      // 연기 초기치
